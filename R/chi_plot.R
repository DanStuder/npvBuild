...
#' @param xlab Bewege die Annotation auf der x-Achse
#' @param ylab Bewege die Annotation auf der y-Achse
#' @importFrom ggplot2 ggplot aes geom_area scale_color_manual geom_vline labs theme_minimal theme annotate element_text
...
chi.plot <- function(chi, df, x, xlab = (x / 15), ylab = -0.001) {
  ...
  dist = dchisq(x = range, df = df)
  H0 = dplyr::if_else(
    range <= qchisq(p = 1 - alpha, df = df, lower.tail = TRUE),
    'Beibehalten',
    'Verwerfen'
  )

  # Create dataframe
  chi.dist <- data.frame(range = range, dist = dist, H0 = H0)

  # Plotting sampling distribution and x_bar value with cutoff
  plot.test <- ggplot(data = chi.dist, aes(x = range, y = dist)) +
    ...
    geom_vline(xintercept = chi, size = 1) +
    annotate(
      "text",
      x = chi + xlab,
      y = 0 - ylab,
